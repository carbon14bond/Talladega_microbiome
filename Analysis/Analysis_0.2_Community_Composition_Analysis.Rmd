...
factorfit(brayscores_16S, samdf_16S$int_eph, strata = samdf_16S$substrate, permutations = 999)
```

```{r}
myColors <- c("#9ACD32","#CD853F","#F4D166")

nmdsp3 <- plot_ordination(pseqtest16S, braymds_16S, type="samples", color="substrate", shape="WetDry")+
...
nmds2plusp16S<-nmds2plus16S+
  geom_point(size=2, aes(shape=WetDry, color=substrate))+
  ...
  stat_ellipse(aes(x=brayscores_16S[,1],y=brayscores_16S[,2],color=samdf_16S$substrate))+
...
nmds2plusleg16S<-nmds2plus16S+
  geom_point(size=2, aes(shape=class_state, color=substrate))+
  scale_colour_manual(values=myColors, name='Substrate:', labels=c('Epilithon', 'Leaf litter', 'Sediment'))+
  #scale_color_discrete(name='Substrate:', labels=c('Epilithic biofilms', 'Leaf litter', 'Benthic sediment'))+
  #scale_shape_discrete(name='Site type:')+
      scale_shape_manual(name='Site type', values = c(1,16,17#,15
                                  ))+
  stat_ellipse(
    #inherit.aes = FALSE, 
               aes(x=brayscores_16S[,1],y=brayscores_16S[,2],color=samdf_16S$substrate, linetype=(samdf_16S$int_eph)))+
  labs(title = "Bray-Curtis NMDS of prokaryote ASVs by substrate")+
  annotate("text", x = 1.5, y = 1, label = sprintf("k=2, Stress=%.4f",braymds_16S$stress), size=3.5)+
...
nmds2pluspITS<-nmds2plusITS+
  ...
  annotate("text", x = -0.5, y = -1.4, label = sprintf("k=2, Stress=%.4f",braymds_ITS$stress), size=3.5)+
 # theme(legend.text = element_text(size=10))+
    theme(text=element_text(size=10), #change font size of all text
        axis.text=element_text(size=10), #change font size of axis text
        axis.title=element_text(size=10), #change font size of axis titles
...
nmds2pluslegITS<-nmds2plusITS+
  ...
  stat_ellipse(
    #inherit.aes = FALSE, 
               aes(x=brayscores_ITS[,1],y=brayscores_ITS[,2],color=samdf_ITS$substrate, linetype=(samdf_ITS$int_eph)))+
  labs(title = "Bray-Curtis NMDS of fungal ASVs by substrate")+
  annotate("text", x = 0.4, y = 0.45, label = sprintf("k=2, Stress=%.4f",braymds_ITS$stress), size=3.5)+
  scale_linetype_discrete(name='Site class:')+
 # theme(legend.text = element_text(size=10))+
    theme(text=element_text(size=10), #change font size of all text
        axis.text=element_text(size=10), #change font size of axis text
...
plotout <- "Beta_sixway_05.13.2025.tiff"
...
L1<- plot_grid(ASVaccumsub16S,vnsp1,
          labels=c('C','D'), ncol=2, rel_widths = c(1.1,1))
L2<- plot_grid(ASVaccumsubITS,vnsp2,
          labels=c('G','H'), ncol=2, rel_widths = c(1.1,1))
...
row1<- plot_grid(R1, LM1, ncol=2, rel_widths = c(1,1))
row2<- plot_grid(R2, LM2,  ncol=2, rel_widths = c(1,1))
...
plotout <- "Overview_05.25.2025.tiff"
agg_tiff(filename=plotout, width=4200, height=3200, units="px",
         pointsize=10, res=600, compression="lzw", scaling=0.5)
bigp
invisible(dev.off())
...
## 0.2.2
### PERMANOVA for microhabitat (substrate) types and intermittent vs. ephemeral
...
adonis2(bray_16S ~ int_eph, strata=samdf_16S$substrate, data = samdf_16S, permutations = 99999, by="terms")
## ITS intermittent versus ephemeral (strata = substrate)
adonis2(bray_ITS ~ int_eph, strata=samdf_ITS$substrate, data = samdf_ITS, permutations = 99999, by="terms")
...
### PERMANOVAs by percent wet
Before PERMANOVA, we want to check for independence of our site variables. 
Note, dry sites lack data for certain variables, and non-STIC sites lack data on others, so we have to subset the data to check differeent combinations of variables.
#### site characteristics
###### ITS sample sets
```{r}
### ITS, all leaf
### subset selecting any one of three substrates to get one row of metadata per site
samdf_ITS_sites<-samdf_ITS[samdf_ITS$substrate=='L',]
samdf_ITS_sites
samdf_ITS_sites$Acer_spp

metasite_corr<- samdf_ITS_sites[,c("S_AFDM_prc","LL_AFDM_prc","elevation","distance_from_outlet","drainage_area_ha","twi", "slope_point","slope_buffer","stream_slope","Canopy_Cover_pct","Quercus_spp","Fagus_grandifolia","Liriodendron_tulipifera","Acer_spp")]

library(linkET)
metacorrplotL<- qcorrplot(correlate(x=metasite_corr[1:14], y=NULL, method = "spearman"), type = "lower", diag = FALSE, grid_size = 0.5) +
  geom_square() +
  ...
  xlab(label="")+
  ylab(label="")+
  labs(title = "Environmental predictors (47 sites)")
metacorrplotL

## Sites with complete dissolved gas data 
metadata_sitecharsa<-samdf_ITS_sites[!is.na(samdf_ITS_sites$CO2_avg),]
metasite_corr<- metadata_sitecharsa[,c("S_AFDM_prc","LL_AFDM_prc","elevation","distance_from_outlet","drainage_area_ha","twi", "slope_point","slope_buffer","stream_slope","Canopy_Cover_pct","Quercus_spp","Fagus_grandifolia","Liriodendron_tulipifera","Acer_spp", "W_Chla", "O2_uM_avg","N2Ar_avg","CO2_avg","N2O_avg", "CH4_avg")]

metacorrplotgas<- qcorrplot(correlate(x=metasite_corr[1:20], y=NULL, method = "spearman"), type = "lower", diag = FALSE, grid_size = 0.5) +
  geom_square() +
  geom_mark(sep = '\n',size=4,sig_level = c(0.05,0.01,0.001),sig_thres = 0.05,
            only_mark = TRUE)+
  #scale_fill_gradientn(colours = RColorBrewer::brewer.pal(11, "RdBu"))+
  ...
  xlab(label="")+
  ylab(label="")+
  labs(title = "Environmental predictors (38 sites)")
metacorrplotgas

## Sites with 28_day wet data
metadata_sitecharsa<-samdf_ITS_sites[!is.na(samdf_ITS_sites$antecedent_28),]
metasite_corr<- metadata_sitecharsa[,c("S_AFDM_prc","LL_AFDM_prc","elevation","distance_from_outlet","drainage_area_ha","twi", "slope_point","slope_buffer","stream_slope","Canopy_Cover_pct","Quercus_spp","Fagus_grandifolia","Liriodendron_tulipifera","Acer_spp", "antecedent_28")]

metacorrplot28<- qcorrplot(correlate(x=metasite_corr[1:15], y=NULL, method = "spearman"), type = "lower", diag = FALSE, grid_size = 0.5) +
  geom_square() +
  geom_mark(sep = '\n',size=4,sig_level = c(0.05,0.01,0.001),sig_thres = 0.05,
            only_mark = TRUE)+
  #scale_fill_gradientn(colours = RColorBrewer::brewer.pal(11, "RdBu"))+
  ...
  xlab(label="")+
  ylab(label="")+
  labs(title = "Environmental predictors (39 sites)")
metacorrplot28

## Sites with 28_day wet data
metadata_sitecharsa<-samdf_ITS_sites[!is.na(samdf_ITS_sites$percentwet_11month),]
metasite_corr<- metadata_sitecharsa[,c("S_AFDM_prc","LL_AFDM_prc","elevation","distance_from_outlet","drainage_area_ha","twi", "slope_point","slope_buffer","stream_slope","Canopy_Cover_pct","Quercus_spp","Fagus_grandifolia","Liriodendron_tulipifera","Acer_spp", "percentwet_11month")]

metacorrplot11<- qcorrplot(correlate(x=metasite_corr[1:15], y=NULL, method = "spearman"), type = "lower", diag = FALSE, grid_size = 0.5) +
  geom_square() +
  geom_mark(sep = '\n',size=4,sig_level = c(0.05,0.01,0.001),sig_thres = 0.05,
            only_mark = TRUE)+
  #scale_fill_gradientn(colours = RColorBrewer::brewer.pal(11, "RdBu"))+
...
library(cowplot)
plotout <- "envi_corrplots_fungi_May132025.tiff"
agg_tiff(filename=plotout, width=5600, height=5600, units="px",
         pointsize=10, res=600, compression="lzw", scaling=0.5)
plot_grid(metacorrplotL, metacorrplotgas, metacorrplot28, metacorrplot11, labels="AUTO", ncol=2)
invisible(dev.off())

## Sites with complete dissolved gas data 
metadata_sitecharsa<-samdf_ITS_sites[!is.na(samdf_ITS_sites$CO2_avg),]
metadata_sitecharsa<-metadata_sitecharsa[!is.na(metadata_sitecharsa$percentwet_11month),]
metasite_corr<- metadata_sitecharsa[,c("S_AFDM_prc","LL_AFDM_prc","elevation","distance_from_outlet","drainage_area_ha","twi", "slope_point","slope_buffer","stream_slope","Canopy_Cover_pct","Quercus_spp","Fagus_grandifolia","Liriodendron_tulipifera","Acer_spp", "W_Chla", "O2_uM_avg","N2Ar_avg","CO2_avg","N2O_avg", "CH4_avg", "percentwet_11month")]

metacorrplotgas11<- qcorrplot(correlate(x=metasite_corr[1:21], y=NULL, method = "spearman"), type = "lower", diag = FALSE, grid_size = 0.5) +
  geom_square() +
  geom_mark(sep = '\n',size=4,sig_level = c(0.05,0.01,0.001),sig_thres = 0.05,
            only_mark = TRUE)+
  #scale_fill_gradientn(colours = RColorBrewer::brewer.pal(11, "RdBu"))+
  ...
  xlab(label="")+
  ylab(label="")+
  labs(title = "Environmental predictors (31 sites)")
metacorrplotgas11

## Sites with complete dissolved gas data 
metadata_sitecharsa<-samdf_ITS_sites[!is.na(samdf_ITS_sites$CO2_avg),]
metadata_sitecharsa<-metadata_sitecharsa[!is.na(metadata_sitecharsa$antecedent_28),]
metasite_corr<- metadata_sitecharsa[,c("S_AFDM_prc","LL_AFDM_prc","elevation","distance_from_outlet","drainage_area_ha","twi", "slope_point","slope_buffer","stream_slope","Canopy_Cover_pct","Quercus_spp","Fagus_grandifolia","Liriodendron_tulipifera","Acer_spp", "W_Chla", "O2_uM_avg","N2Ar_avg","CO2_avg","N2O_avg", "CH4_avg", "antecedent_28")]
...
metacorrplotgas11<- qcorrplot(correlate(x=metasite_corr[1:21], y=NULL, method = "spearman"), type = "lower", diag = FALSE, grid_size = 0.5) +
  ...
  xlab(label="")+
  ylab(label="")+
  labs(title = "Environmental predictors (30 sites)")
metacorrplotgas11

##### biofilms chlorophyll ##########
metadata_sitecharsa<-samdf_ITS_sites[!is.na(samdf_ITS_sites$B_Chla),]
metadata_sitecharsa<-metadata_sitecharsa[!is.na(metadata_sitecharsa$percentwet_11month),]
metasite_corr<- metadata_sitecharsa[,c("S_AFDM_prc","LL_AFDM_prc","elevation","distance_from_outlet","drainage_area_ha","twi", "slope_point","slope_buffer","stream_slope","Canopy_Cover_pct","Quercus_spp","Fagus_grandifolia","Liriodendron_tulipifera","Acer_spp", "B_Chla", "percentwet_11month")]
metacorrplotgas11<- qcorrplot(correlate(x=metasite_corr[1:16], y=NULL, method = "spearman"), type = "lower", diag = FALSE, grid_size = 0.5) +
  geom_square() +
  geom_mark(sep = '\n',size=4,sig_level = c(0.05,0.01,0.001),sig_thres = 0.05,
            only_mark = TRUE)+
  #scale_fill_gradientn(colours = RColorBrewer::brewer.pal(11, "RdBu"))+
  ...
  xlab(label="")+
  ylab(label="")+
  labs(title = "Environmental predictors (35 sites)")
metacorrplotgas11

metadata_sitecharsa<-samdf_ITS_sites[!is.na(samdf_ITS_sites$B_Chla),]
metadata_sitecharsa<-metadata_sitecharsa[!is.na(metadata_sitecharsa$antecedent_28),]
metasite_corr<- metadata_sitecharsa[,c("S_AFDM_prc","LL_AFDM_prc","elevation","distance_from_outlet","drainage_area_ha","twi", "slope_point","slope_buffer","stream_slope","Canopy_Cover_pct","Quercus_spp","Fagus_grandifolia","Liriodendron_tulipifera","Acer_spp", "B_Chla", "antecedent_28")]
metacorrplotgas11<- qcorrplot(correlate(x=metasite_corr[1:16], y=NULL, method = "spearman"), type = "lower", diag = FALSE, grid_size = 0.5) +
  geom_square() +
  geom_mark(sep = '\n',size=4,sig_level = c(0.05,0.01,0.001),sig_thres = 0.05,
            only_mark = TRUE)+
...
####### percent wet, dissolvedd gas, and epilithic chlorophyll ###########
## Sites with complete dissolved gas data 
metadata_sitecharsa<-samdf_ITS_sites[!is.na(samdf_ITS_sites$CO2_avg),]
metadata_sitecharsa<-metadata_sitecharsa[!is.na(metadata_sitecharsa$percentwet_11month),]
metadata_sitecharsa<-metadata_sitecharsa[!is.na(metadata_sitecharsa$B_Chla),]

metasite_corr<- metadata_sitecharsa[,c("S_AFDM_prc","LL_AFDM_prc","elevation","distance_from_outlet","drainage_area_ha","twi", "slope_point","slope_buffer","stream_slope","Canopy_Cover_pct","Quercus_spp","Fagus_grandifolia","Liriodendron_tulipifera","Acer_spp", "W_Chla", "O2_uM_avg","N2Ar_avg","CO2_avg","N2O_avg", "CH4_avg", "percentwet_11month","B_Chla")]

metacorrplotgas11<- qcorrplot(correlate(x=metasite_corr[1:22], y=NULL, method = "spearman"), type = "lower", diag = FALSE, grid_size = 0.5) +
  geom_square() +
  geom_mark(sep = '\n',size=4,sig_level = c(0.05,0.01,0.001),sig_thres = 0.05,
            only_mark = TRUE)+
  #scale_fill_gradientn(colours = RColorBrewer::brewer.pal(11, "RdBu"))+
  ...
  xlab(label="")+
  ylab(label="")+
  labs(title = "Environmental predictors (28 sites)")
metacorrplotgas11
library(cowplot)
plotout <- "meta_STIC_bchla_gas_May172025.tiff"
agg_tiff(filename=plotout, width=3000, height=3000, units="px",
         pointsize=10, res=600, compression="lzw", scaling=0.5)
plot_grid(metacorrplotgas11)
invisible(dev.off())
...
### 28-day percent wet
pseqITS_28<- subset_samples(pseqtestITS, !is.na(antecedent_28))
bray_ITS_28 <- phyloseq::distance(pseqITS_28, method = "bray")
#braymds_ITS_28 <- metaMDS(bray_ITS_28, k=2, trymax=500, wascores = T)
#brayscores_ITS_28 <- scores(braymds_ITS_28)
sam_ITS_28 <- data.frame(sample_data(pseqITS_28 ))
adonis2(bray_ITS_28 ~ antecedent_28, strata=sam_ITS_28$substrate, data = sam_ITS_28, permutations = 999, by="terms")

### 11-month percent wet
pseqITS_11<- subset_samples(pseqtestITS, !is.na(percentwet_11month))
bray_ITS_11 <- phyloseq::distance(pseqITS_11, method = "bray")
#braymds_ITS_11 <- metaMDS(bray_ITS_11, k=2, trymax=500, wascores = T)
#brayscores_ITS_11 <- scores(braymds_ITS_11)
sam_ITS_11 <- data.frame(sample_data(pseqITS_11 ))
adonis2(bray_ITS_11 ~ percentwet_11month, strata=sam_ITS_11$substrate, data = sam_ITS_11, permutations = 999, by="terms")

### dissoved Oxygen (excluding dry sites)
pseqITS_O2<- subset_samples(pseqtestITS, !is.na(O2_uM_avg))
bray_ITS_O2 <- phyloseq::distance(pseqITS_O2, method = "bray")
#braymds_ITS_O2 <- metaMDS(bray_ITS_O2, k=2, trymax=500, wascores = T)
#brayscores_ITS_O2 <- scores(braymds_ITS_O2)
sam_ITS_O2 <- data.frame(sample_data(pseqITS_O2 ))
adonis2(bray_ITS_O2 ~ O2_uM_avg, strata=sam_ITS_O2$substrate, data = sam_ITS_O2, permutations = 999, by="terms")
